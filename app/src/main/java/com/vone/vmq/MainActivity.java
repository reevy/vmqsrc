...
public class MainActivity extends AppCompatActivity{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Toast.makeText(MainActivity.this, "VMQ免签系统 v1.9", Toast.LENGTH_SHORT).show();
    }
    ...
    public void doInput(View v){
        ...
        builder.setPositiveButton("确认", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                String[] cfgUrl = scanResult.split("//");
                if (cfgUrl.length!=2){
                    Toast.makeText(MainActivity.this, "数据错误，需要带协议的完整URL!", Toast.LENGTH_SHORT).show();
                } else {
                    String[] tmp = cfgUrl[1].split("/");
                    if (tmp.length!=2){
                ...
                Request request = new Request.Builder().url(cfgUrl[0]+"://"+tmp[0]+"/vmq/heart?t="+t+"&sign="+sign).method("GET",null).build();
    ...
    public void doStart(View view) {
        ...
        OkHttpClient okHttpClient = new OkHttpClient();
        Request request = new Request.Builder().url(host+"/vmq/heart?t="+t+"&sign="+sign).method("GET",null).build();
        Call call = okHttpClient.newCall(request);
        call.enqueue(new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == Constant.REQ_QR_CODE && resultCode == RESULT_OK) {
            ...
            OkHttpClient okHttpClient = new OkHttpClient();
            Request request = new Request.Builder().url("http://"+tmp[0]+"/vmq/heart?t="+t+"&sign="+sign).method("GET",null).build();
            Call call = okHttpClient.newCall(request);
            call.enqueue(new Callback() {
                @Override
                public void onFailure(Call call, IOException e) {}
